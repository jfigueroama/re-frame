(ns todomvc.subs
  (:require [re-frame-lib.core :refer [reg-sub subscribe]]))
...
(defn register
  ...
  (-> state
      ...
      (reg-sub
        ...
        (fn [query-v _]
          (subscribe state [:sorted-todos]))    ;; returns a single input signal
      ...
      (reg-sub
        ...
        (fn [query-v _]
          [(subscribe state [:todos])
           (subscribe state [:showing])])
