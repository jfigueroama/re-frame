(ns todomvc.events
  (:require
    ...
    [re-frame-lib.core :refer [reg-event-db reg-event-fx inject-cofx path after]]
...
(defn ->local-store [state] (after (partial todos->local-store state)))
...
(defn register
  ...
  ;; registration.
  (let [todo-interceptors [check-spec-interceptor    ;; ensure the spec is still valid  (after)
    ...
    (-> state
      (reg-event-fx                 ;; part of the re-frame API
        ...
        [(inject-cofx state :local-store-todos) ;; gets todos from localstore, and puts value into coeffects arg
