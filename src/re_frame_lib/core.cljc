(ns re-frame-lib.core
  (:require
    ...
    [re-frame-lib.interop          :as interop :refer [empty-queue ratom]]
...
(defn dispatch
  ...
  Usage:
      ...
      #!clj
      (dispatch state [:order \"pizza\" {:supreme 2 :meatlovers 1 :veg 1}])
  "
  {:api-docs/heading "Dispatching Events"}
  [state event]
  (router/dispatch state event))
...
(defn dispatch-sync
  ...
  Usage:
      ...
      #!clj
      (dispatch-sync state [:sing :falsetto \"piano accordion\"])
  "
  {:api-docs/heading "Dispatching Events"}
  [state event]
  (router/dispatch-sync state event))
...
(defn reg-event-db
  ...
  ([state id interceptors handler]
   (events/register state id [(cofx/inject-db state) (fx/do-fx state) (std-interceptors/inject-global-interceptors state) interceptors (db-handler->interceptor handler)])))
...
(defn reg-sub
  ...
  Returns the application's state with the subscription registered.
  ...
  **Third variation** - syntax Sugar
  ...
  [state query-id & args]
  (apply subs/reg-sub (into [state query-id] args)))
...
(defn subscribe
  ...
  **Hint**
  ...
  In fact, to avoid any mistakes, some prefer to define:
      ...
      #!clj
      (def <sub state (comp deref re-frame.core/subscribe))
  ...
  **De-duplication**
  ...
  {:api-docs/heading "Subscriptions"}
  ([state query]
   (subs/subscribe state query))
  ([state query dynv]
   (subs/subscribe state query dynv)))

(def clear-sub ;; think unreg-sub
...
(defn reg-fx
  ...
  [state id handler]
  (fx/reg-fx state id handler))


(def clear-fx ;; think unreg-fx
...
(defn reg-cofx
  ...
  [state id handler]
  (cofx/reg-cofx state id handler))
...
(defn inject-cofx
  ...
  **Background**
  ...
  {:api-docs/heading "Coeffects"}
  ([state id]
   (cofx/inject-cofx state id))
  ([state id value]
   (cofx/inject-cofx state id value)))

(def clear-cofx ;; think unreg-cofx
...
(defn reg-global-interceptor
  ...
  [state interceptor]
  (settings/reg-global-interceptor state interceptor))
...
(defn clear-global-interceptor
  ...
  {:api-docs/heading "Global Interceptors"}
  ([state]
   (settings/clear-global-interceptors state))
  ([state id]
   (settings/clear-global-interceptors state id)))
...
(defn purge-event-queue
  ...
  {:api-docs/heading "Miscellaneous"}
  [{:keys [event-queue] :as state}]
  {:pre [(state? state)]}
  (router/purge event-queue))
...
(defn add-state-defaults
  ...
  (-> state
      ...
      (reg-fx
        ...
        (fn [value]
          (if (map? value)
            (fx/dispatch-later state value)
            (doseq [effect (remove nil? value)]
              (fx/dispatch-later state effect)))))
